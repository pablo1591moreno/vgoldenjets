...
const legs = blocks.map((lines, idx) => {
    ...
    if (routeMatch) {
        const fromCity = routeMatch[1].replace(/,$/, '').trim();
    ...
    // Capture full availability string
    const availabilityRaw = availLine.replace(/^Disponible:\s*/i, "").trim();

    // Still parse date for sorting/logic if needed, but not strictly for display now
    const availText = availabilityRaw.split("–")[0].trim();
    ...
    return {
        ...
        to,
        date,
        availability: availabilityRaw, // NEW FIELD
        aircraft,
        seats,
        imageVar
    };
...
const outputContent = `// Interface is defined below
${imports}
...
console.log(`Parsed ${legs.length} legs with availability.`);
