...
const ArticleCarousel = () => {
    ...
    // Sort articles by date (newest first) and take the first 6
    ...
    const sectionTitle = isEN ? "Latest News" : "Últimas Novedades";
    const sectionSubtitle = isEN
        ? "Stay up to date with the world of private aviation"
        : "Manténgase al día con el mundo de la aviación privada";
        ...
        <section className="bg-black py-16 border-t border-white/10">
            <div className="section-container">
                <div className="flex justify-between items-end mb-8">
                    <div>
                        <h2 className="section-title font-serif text-white mb-2">{sectionTitle}</h2>
                        <p className="section-subtitle" style={{ color: "rgb(234 213 155 / 0.9)" }}>{sectionSubtitle}</p>
                    </div>
                ...
                <div className="overflow-hidden" ref={emblaRef}>
                    <div className="flex -ml-4">
                        {articles.map((article) => {
                            ...
                            const date = new Date(article.dateMs).toLocaleDateString(isEN ? 'en-US' : 'es-AR', {
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric'
                            });
