...
const DestinationCard = ({ route }: { route: string }) => {
  return (
    <div className="bg-black/80 p-3 sm:p-4  hover:bg-black/60 transition-colors flex items-center space-x-3 rounded-xl">
      <Plane size={18} className="text-gold" />
      <span className="text-white text-base sm:text-lg font-medium">{route}</span>
    </div>
  );
};
